...
class DatabaseCartRepositoryTest {
    ...
    void shouldSaveCartItemsUsingBatch() throws Exception {
        ...
        try (MockedStatic<DatabaseConnection> mocked =
            ...
            doNothing().when(stmt).addBatch();            when(stmt.executeBatch())
            ...
            verify(stmt).setInt(1, 1); // cart_id set once

            verify(stmt, times(2)).setDouble(eq(2), anyDouble());  // item value
            verify(stmt, times(2)).setInt(eq(3), anyInt());        // item quantity
